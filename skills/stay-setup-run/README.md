...
Outputs land next to the guide folder in `<share_folder>_실행/`: `steps.json`, the minified helper files,
and `run-log.md` (one line per step, plus a closing section listing guide defects and any dangerous button
a supervisor pressed).
