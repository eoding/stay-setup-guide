...
- **`--check` 를 먼저 돌린다.** 0 이 아니면 브라우저를 열지 않는다. 걸리는 것은 넷이다:
  ...
  **`줄 차례가 어긋난 단계`**(부과금의 `연령별 단가` 가 `부과 방식` 보다 앞에 있는 꼴).
...
5. 저장:
   - **룸 사진은 [저장] 을 누르지 않는다.** 파일을 고르는 순간 올라간다(`hx-trigger="change"`).
     `takeFiles` 가 `autoUpload: true` 와 `uploaded` 를 돌려준다 — `uploaded` 가 참이면 끝난 것이고,
     여기서 [저장] 을 누르면 룸 폼이 저장되며 드로어가 닫힌다(사진과 상관없는 버튼이다).
   - 이미지 모달은 헤더 [저장] 을 장수만큼 누른다: `stayRun.submit('저장')`.
...
- **`연령별 단가` 의 비움과 0 은 뜻이 다르다.** 비움은 성인가와 같다는 뜻이고 `0` 은 무료다. 지시서에 `0` 이 있으면 `0` 을 넣는다. 그 카드는 `부과 방식` 이 `인당 · 정액` 이고 요율 행이 0일 때만 서므로 **줄 차례를 바꾸지 않는다.**
...
9. 룸 사진 단계의 저장 글자가 [저장] 으로 적혀 있다 → 룸 사진에는 저장 버튼이 없다(고르면 올라간다). 저장 줄을 빼야 한다.
10. 부과금 단계의 `연령별 단가` 줄이 `부과 방식` 보다 앞에 있거나 요율 행 뒤에 있다 → 그 차례로는 칸이 화면에 서지 않는다.
