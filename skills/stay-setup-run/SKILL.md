...
> - **CSRF 403 은 한 번만 스스로 다시 보낸다.** 오래 열려 있던 탭의 첫 저장이 403 으로
>   거부되던 것을 러너가 넘긴다 — 새 `csrftoken` 을 읽어 헤더를 덮어쓰고 같은 저장을
>   **딱 한 번** 다시 보낸다. 성공하면 `csrfRetried: true` 이고 실패로 세지 않는다.
>   다른 4xx·5xx 는 재시도하지 않는다.
>
...
>
> **운영 2026-09-15 배포판 화면 기준.** 새 호텔은 **오퍼 0 · 룸 0** 으로 시작한다.
> 그 전 화면을 보는 옛 지시서에서는 v0.2.x 를 쓴다 — 이 러너는 옛 화면 기준 단계를 만나면
> **거부하고 멈춘다**(`window.checkGuide()` 로 실행 전에 미리 훑을 수 있다).
>
> **2026-09-08 에 바뀐 것** (자세한 것은 `references/screen-mechanics.md`):
> - 시즌 `가격 채우기` 드로어에 칸 둘이 생겼다 — **`박수별 단가(선택)`**(`3:100,5:90`, 부호 없음 ·
>   박수 2 이상)와 **`아동 추가 금액 — <노출명>`**(유료 연령 구간마다 한 칸, 구분자는 EM DASH).
>   **아이가 방에서 자는 1박 요금의 자리는 그 칸 하나뿐이다** — 부가옵션으로 소아 숙박을 팔지 않는다.
> - `아동 추가 금액` 칸이 서려면 그 오퍼의 **`연령 구간 만들기` 가 `시즌 가격 채우기` 보다 앞**이어야 한다.
> - `성인 요금의 %` 구간의 아동 금액 칸은 `disabled`+`readonly` 다 — 지시서의
>   `자동 입력됨 · 그대로 둠` 을 러너가 **건드리지 않고 지나간다**.
> - `인원 조합` 은 **A-형 좌표**(`A2,A3` · `A2C1_CHD`)만 쓴다. 옛 숫자 키는 `--check` 가 막는다.
> - `밴드 코드` 는 **영대문자·숫자 2~8자**(`CHD`·`INF`·`TEEN`)다 — 밑줄은 가격 셀 좌표를 갈라 놓는다.
> - 가격 셀 편집 모달: `출처` 열이 없어지고 **`박수~` 열**이 돌아왔으며, 아래 카드가
>   **`인원별 · 박수별 가격 추가`** 로 바뀌고 그 `인원 조합` 이 **셀렉트**가 됐다
>   (항목 글자는 키가 앞 — `A2C1_CHD · 성인 2 · 소아 1`).
>
> **2026-09-09 에 바뀐 것** (같은 문서의 「셀 편집 창 — 좌표별 묶음 표의 속살」 절):
> - 셀 편집 창의 위 표에서 `인원 조합` 칸이 그룹 머리의 `rowspan` 으로 층 전체를 덮는다 —
>   **층 둘째 행부터는 `<td>` 가 하나 적다.** 자리로 세면 `판매가` 값이 `정가` 칸에 들어가고
>   저장까지 간다. 도우미 v0.6+ 가 `rowCells` 로 격자를 풀어 **열 번호**로 잡는다.
...
>   수 없습니다` · `박수는 30 이하여야 합니다` 등이다.
> - 부과금 드로어: 요율 행이 있으면 `연령별 단가` 자리에 표 대신 **안내 카드**가 선다(채울 칸이
>   없다). 그 자리에서 `not-found` 가 나오면 값이 아니라 **줄 차례**를 의심한다.
> - 표 액션 버튼이 `td.td_actions > div.stay-actions` 안으로 한 겹 더 들어갔다 — 찾는 길은 그대로다.
> - 목록에 [AI 에이전트로 세팅], 편집 화면에 [에이전트에 복사] 모달이 생겼다(둘 다 `hidden` 으로
>   늘 실려 있다). **러너는 열지도 누르지도 않는다** — 사람에게 줄 문구를 담은 화면이다.
> - 좌표 꼴 규칙이 `--check` 로 내려왔다: 32자 상한, 첫 아동 조각은 성인 자리에 붙여 쓰기
>   (`A2C1_CHD`, `A2_C1_CHD` 아님), 코드 없는 조각은 `A2C1` 한 모양뿐(혼합 `A2C1_C1_CHD` 거부),
>   밴드 코드 `C1` 꼴 거부. 서버 파서와 같은 판정으로 맞춰 두었다.
> - 직원 예약 화면(러너가 안 만진다): 아동 나이가 **만 0~17세**로 넓어졌고 선택지 글자가
>   `만 N세` 다. 박수 층이 모자란 요청은 `<N>박은 이 요금제가 파는 최소 숙박일수에 미달합니다`
>   로 마감된다 — 세팅 구멍이 아니라 층 구성의 결과다.
>
> **2026-09-15 에 바뀐 것**:
> - 프로모션 드로어에 `진행기간 제한`(체크박스) · `진행기간 시작` · `진행기간 종료`(날짜 고르개)
>   세 칸이 `적용 숙박일 종료` 바로 아래 생겼다 — **예약하는 날** 축이라 `적용 숙박일`(묵는 날)과
>   다르다. 종류가 이미 다루는 것들이라 러너에 새 갈래는 없다. 제한을 켠 지시서는 두 날짜가
>   모두 있어야 저장된다(한쪽만 채우면 드로어가 거부한다).
> - 목록 행에 노란 배지 「예약기간 전」·「예약기간 지남」 이 붙을 수 있다 — **설정 오류가 아니라**
>   지금이 그 예약 기간 밖이라는 표시다. 러너가 멈출 이유가 아니다.
> - 어메니티 카드의 `다른 룸에서 복사…` 셀렉트에 `aria-label="다른 룸에서 복사"` 가 붙었다.
>   걸러 내는 판정은 **첫 옵션 글자**를 먼저 보므로 도우미 동작은 그대로다.
> - 객실 목록 `사진` 열의 0 은 레거시 공유 카테고리에서만 나던 것이고 고쳐졌다
>   (`references/screen-mechanics.md` §객실 탭).
...
2. 사용자가 로그인해 둔 크롬 탭. 호텔 목록 화면에서 시작하거나, 이미 만든 호텔의 편집 화면에서 이어서 한다. **주소는 사용자에게 받는다 — 이 스킬은 주소를 모른다.**
   ...
   PC 앞에 없는 사용자는 원격 데스크톱 등 **자기 도구**로 그 탭에 넣는다 — 사용자가 채팅에 숫자만 적으면 넣지 않고 그렇게 안내한다.
...
- **`--check` 를 먼저 돌린다.** 0 이 아니면 브라우저를 열지 않는다. **2 는 문지기가 막은 것**이다 —
  ...
  `검사 도장: ok · sha256 … · 렌더 …` 한 줄로 먼저 찍는다). 1 로 걸리는 것은 일곱이다:
  ...
  **`값 형식이 어긋난 단계`**(2026-09-08: `박수별 단가` 가 `박수:1박 단가` 가 아니거나 부호가
  붙었거나 박수가 1 이하·중복 · `인원 조합`·`인원 조합별 조정` 에 옛 숫자 키(`2,3`·`3:+14`) ·
  `밴드 코드` 가 `^[A-Z0-9]{2,8}$` 가 아님) ·
  **`단계 차례가 어긋난 단계`**(2026-09-08: `아동 추가 금액` 줄이 있는 `시즌 가격 채우기` 가
  그 `연령 구간 만들기` 보다 앞 — 유료 구간이 없으면 그 칸이 화면에 서지 않는다).
...
stayRun.submit("저장")             // {status:"closed|stayed|error|navigated|login|timeout|refused|not-found|upload-label", errors, toast}
                                   ...
                                   // **CSRF 403 이면 토큰을 다시 읽어 한 번만 재시도한다**(0.9.3) —
                                   // 통하면 csrfRetried:true, 두 번째도 403 이면 csrfRetryFailed:true 에 status:'error'
stayRun.refreshCsrf()              // 새 CSRF 토큰을 쿠키(없으면 화면)에서 읽어 이후 요청 헤더에 덮어쓴다
                                   // {ok, from:"cookie|dom", inputs, hooked} — **토큰 값 자체는 돌려주지 않는다**
                                   // submit 이 403 에서 스스로 부른다. 손으로 부를 일은 드물다(0.9.3)
...
- **`submit: 'error'` 에 `(403)` 이 붙어 있으면** CSRF 토큰이 낡은 것이다. 러너가 이미 새 토큰으로
  **한 번** 다시 보내 본 뒤다(`csrfRetryFailed: true`) — 두 번째도 막혔으면 세션이 끊긴 것에 가깝다.
  스크린샷으로 로그인 화면인지 보고, 사용자에게 탭을 새로 고쳐 다시 로그인해 달라고 청한다.
  **같은 저장을 손으로 여러 번 누르지 않는다.**
  재시도가 통한 단계는 결과에 `csrfRetried: true` 가 실린다 — 로그 비고에 `403 재시도 후 저장` 을
  적고 완료로 센다(저장은 제대로 된 것이다).
