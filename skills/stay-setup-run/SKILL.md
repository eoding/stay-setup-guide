...
- **캠페인 만들기** — 상품관리 목록이 아니라 **Stay 호텔 목록 화면의 [캠페인]** → 캠페인 목록 화면의 [캠페인 만들기] 드로어 → fill → [추가]. 호텔을 만들기 전에 캠페인이 필요하면 이 단계를 먼저 한다.
...
- **호텔 만들기 전에 지시서의 `공급 통화` 를 본다.** USD 가 아니면 호텔을 만들지 말고 사용자에게 그 통화가 맞는지 확인받는다(통화는 나중에 [환율 설정] 탭에서 바꿀 수 있지만 숫자는 그대로 남아 모든 단가를 다시 깔아야 한다).
...
6. 캠페인 버튼 위치가 상품관리 목록으로 적혀 있다 → Stay 호텔 목록 화면이다.
7. 시즌 드로어의 저장 버튼은 [추가] 인데 지시서는 [저장] 으로 적는다.
