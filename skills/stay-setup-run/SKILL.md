...
2. 사용자가 로그인해 둔 크롬 탭. 호텔 목록 화면에서 시작하거나, 이미 만든 호텔의 편집 화면에서 이어서 한다. **주소는 사용자에게 받는다 — 이 스킬은 주소를 모른다.**
   ...
   PC 앞에 없는 사용자는 인증앱 숫자를 **`! bash skills/stay-setup-run/scripts/mfa_type.sh 123456`** 처럼 `!` 명령으로 던진다 — 그 명령이 크롬 창을 앞으로 올려 코드를 치고 [인증] 을 누른다(사용자 셸에서 도는 사용자 명령이다). 사용자가 채팅에 숫자만 적으면 이 한 줄을 그대로 안내한다. **에이전트는 이 스크립트를 실행하지 않는다.**
