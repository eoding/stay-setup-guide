...
- **새 호텔은 오퍼 0 · 룸 0 이다**(2026-09-04). 첫 오퍼도 [오퍼 추가], 1번째 룸도 [룸 추가] 로 만든다 — 고쳐 쓸 `기본 오퍼`·`스탠다드` 는 이제 없다. 눈에 보이지 않는 필수 레코드와 `기본 요금제` **정본** 은 그대로 생기므로, 요금제는 여전히 새로 만들지 말고 정본을 고쳐 쓴다.
